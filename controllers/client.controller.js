...
export const createClient = async (req, res) => {console.log(req.body)
...
function generateRefreshToken(client) {
 return jwt.sign({client}, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '1y' });
}
//Refresh
export const RefreshToken = async (req, res) => {
 ...
 const{email,password}=req.body;
 const client = await Client.findOne({email});
 if(client==""){ res.status(404).send('utilisateur non existant');return ;};
 const isMatch=await bcrypt.compare(password,client.password);
 if(!isMatch) res.status(400).json({msg:'mot de passe incorrect'})
...
export const getclientBYEmailClient = async (req, res) => {
 ...
 const{email,password}=req.body;
 const client = await Client.findOne({email});
 if(client==""){ res.status(401).send('utilisateur non existant');return ;};
 const isMatch=await bcrypt.compare(password,client.password);
 if(!isMatch) res.status(400).json({msg:'mot de passe incorrect'})
 const accessToken = generateAccessToken(client);
 const refreshToken = generateRefreshToken(client);
